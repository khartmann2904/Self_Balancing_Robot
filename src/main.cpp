...
// offset for the IMU pitch angle (if needed)
#define IMU_PITCH_OFFSET 1.0f // Adjust this value based on calibration
...
void loop() {
    ...
    if ((now - lastControlTime) >= 5000UL) {
        ...
        imu.update();
        float currentAngle = imu.getPitch(); // Adjust for IMU offset if necessary

        // Safety cutoff in case of a fall or emergency button press (> 45 degrees)
        if (battery.isBatteryLow() || bluetooth.isEmergencyStopPressed() || abs(currentAngle) > 45.0f) {
        ...
        // Compute the motor command using the cascade control loop
        float motorCommand = controller.computeCascade(targetSpeed, currentSpeed, currentAngle, dt);
        Serial.println("Motor Command: " + String(motorCommand));
        motors.setSpeeds(-motorCommand, motorCommand);  // Set speeds for left and right motors (inverted for one side)
    }
}